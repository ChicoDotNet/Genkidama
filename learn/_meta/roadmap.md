...
**PowerShell está en progreso en PR #70 con 12/17 lecciones.** WorkstationAudit ya cubre diagnóstico portable, configuración validada, CIM de Windows detrás de una frontera explícita, reglas de almacenamiento/memoria, inventario de software/servicios acotado, contexto de privilegios, reportes JSON y texto derivados del mismo objeto, persistencia e identificación semántica de cambios entre auditorías. La validación real Windows detectó entradas de desinstalación sin `DisplayName`; el filtro ahora comprueba la existencia de la propiedad bajo StrictMode en vez de debilitarlo. Tres checkpoints están completos y el slice 9–12 está verde en Windows y Ubuntu. El siguiente bloque 13–16 cubrirá remoting y alcance seguro, concurrencia sólo donde aporte valor, debugging/profiling y hardening/entrega. No se abre Ruby mientras PowerShell siga incompleto salvo motivo documentado.
