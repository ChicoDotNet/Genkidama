import { COLUMN_IDS, filterCards } from "./board.js";
import { applyBoardCommand } from "./commands.js";
...
import { assertImportFileSize, exportBoard, importBoard } from "./storage.js";
...
async function persistAndRender(message) {
  persistenceMode = await diagnostics.measureAsync("persistence.save", () =>
    savePreferredBoard({
      indexedDbFactory: window.indexedDB,
      storage: window.localStorage,
      board,
      saveIndexedDb: saveBoardToIndexedDb,
    }),
  );
  render();
...
form.addEventListener("submit", async (event) => {
  ...
  try {
    board = applyBoardCommand(board, { type: "add", title: titleInput.value });
    form.reset();
    titleInput.focus();
    await persistAndRender("Tarjeta agregada.");
  } catch (error) {
...
boardElement.addEventListener("click", async (event) => {
  ...
  try {
    if (button.dataset.action === "move") {
      board = applyBoardCommand(board, {
        type: "move",
        cardId,
        targetColumn: button.dataset.moveTo,
      });
      await persistAndRender("Tarjeta movida.");
      return;
    }
    ...
    if (button.dataset.action === "edit") {
      ...
      board = applyBoardCommand(board, { type: "edit", cardId, title });
      await persistAndRender("Tarjeta editada.");
      return;
    }

    if (button.dataset.action === "delete") {
      if (!window.confirm(`¿Eliminar "${currentCard.title}"?`)) return;
      board = applyBoardCommand(board, { type: "delete", cardId });
      await persistAndRender("Tarjeta eliminada.");
    }
  } catch (error) {
    errorElement.textContent = error.message;
  }
...
importInput.addEventListener("change", async () => {
  ...
  try {
    ...
    board = diagnostics.measure("import.parse", () => importBoard(text));
    searchInput.value = "";
...
async function initialize() {
  const loaded = await diagnostics.measureAsync("persistence.load", () =>
    loadPreferredBoard({
      indexedDbFactory: window.indexedDB,
      storage: window.localStorage,
      loadIndexedDb: loadBoardFromIndexedDb,
    }),
  );
  board = loaded.board;
  persistenceMode = loaded.mode;

  if (loaded.mode === "localstorage" && board.cards.length > 0) {
    persistenceMode = await diagnostics.measureAsync("persistence.migrate", () =>
      savePreferredBoard({
        indexedDbFactory: window.indexedDB,
        storage: window.localStorage,
        board,
  ...
  render();
  setStatus(`Tablero listo. Persistencia: ${persistenceMode}.`);

  if ("serviceWorker" in navigator) {
    try {
      await navigator.serviceWorker.register("./service-worker.js");
    } catch {
      setStatus(`Tablero listo. Persistencia: ${persistenceMode}. El modo offline no pudo registrarse.`);
    }
  }
}

initialize().catch((error) => {
  errorElement.textContent = `No se pudo iniciar el tablero: ${error.message}`;
});
