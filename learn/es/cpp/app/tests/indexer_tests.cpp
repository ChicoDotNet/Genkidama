...
void indexes_regular_files_and_totals_bytes() {
    ...
    expect(index.files().front().path.filename() == "README.md", "debe ordenar por ruta");
}

void searches_filename_case_insensitively() {
    const TempDirectory fixture;
    const threadseek::FileIndex index(fixture.path());
    const auto matches = index.search("txt");

    expect(matches.size() == 2, "debe encontrar dos nombres .txt sin importar mayúsculas");
}

void empty_query_returns_all_files() {
    const TempDirectory fixture;
    const threadseek::FileIndex index(fixture.path());

    expect(index.search("").size() == index.files().size(), "consulta vacía debe devolver todo el índice");
}
...
int main() {
    ...
    empty_query_returns_all_files();
    rejects_missing_root();

    if (failures == 0) {
        std::cout << "4 pruebas pasaron\n";
        return 0;
    }
