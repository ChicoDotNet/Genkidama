...
func TestHandlerAddsDefensiveHeadersAndRejectsUnknownRoutes(t *testing.T) {
	...
	if health.Header().Get("Referrer-Policy") != "no-referrer" {
...
func TestDiagnosticsAreOptInAndAggregateOnly(t *testing.T) {
	...
	if disabled.Code != http.StatusNotFound {
		t.Fatalf("disabled diagnostics status = %d", disabled.Code)
	}
	...
	clock := func() time.Time {
		current := base.Add(time.Duration(step) * 25 * time.Millisecond)
		step++
		return current
	}
	server, err := NewServerWithDiagnostics(
		fakeChecker{err: errors.New("network unavailable")},
		[]monitor.Target{{Name: "private-api", URL: "https://secret.example.test/path"}},
		1,
		nil,
		metrics,
		clock,
	...
	if failed.Code != http.StatusServiceUnavailable {
		t.Fatalf("failed check status = %d", failed.Code)
	}
	...
	if strings.Contains(raw, "private-api") || strings.Contains(raw, "secret.example.test") {
		t.Fatalf("diagnostics leaked target data: %s", raw)
	}
	var got MetricsSnapshot
	if err := json.Unmarshal([]byte(raw), &got); err != nil {
		t.Fatal(err)
	}
	if got.Requests != 1 || got.Failures != 1 || got.TotalDurationMillis != 25 {
