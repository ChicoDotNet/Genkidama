...
sealed class StyleFactory
{
    ...
    public TextStyle Get(string font, int size, string color)
    {
        ...
        if (!_styles.TryGetValue(key, out var style))
            _styles[key] = style = new TextStyle(font, size, color);
        return style;
    }
...
var glyphs = new[] { new Glyph('A', 1, red1), new Glyph('B', 2, red2), new Glyph('C', 3, blue) };
Console.WriteLine($"styles={factory.Count};shared={ReferenceEquals(glyphs[0].Style, glyphs[1].Style).ToString().ToLowerInvariant()};text={string.Concat(Array.ConvertAll(glyphs, g => g.Character))}");
