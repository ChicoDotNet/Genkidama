...
struct BalanceCommand
  ...
  def execute(balance : Int32) : Int32; balance + delta; end
  def undo(balance : Int32) : Int32; balance - delta; end
end
def command_pattern
...
class Literal < Expr
  ...
  def eval : Int32; @value; end
end
class AddExpr < Expr
  def initialize(@left : Expr, @right : Expr); end
  def eval : Int32; @left.eval + @right.eval; end
end
class MulExpr < Expr
  def initialize(@left : Expr, @right : Expr); end
  def eval : Int32; @left.eval * @right.eval; end
...
class CursorIterator
  def initialize(@values : Array(Int32)); @index = 0; end
...
class Editor
  ...
  def save : EditorMemento; EditorMemento.new(@state); end
  def restore(m : EditorMemento); @state = m.state; end
end
def memento_pattern
...
# Observer
class Subject
  def initialize; @observers = [] of Proc(Int32, String); end
  def subscribe(observer : Proc(Int32, String)); @observers << observer; end
  def publish(id : Int32) : Array(String); @observers.map { |o| o.call(id) }; end
end
...
# Strategy
def price(value : Int32, strategy : Proc(Int32, Int32)) : Int32; strategy.call(value); end
...
class Circle < Shape
  ...
  def accept(visitor : ShapeVisitor) : Float64; visitor.visit_circle(@radius); end
end
class Rectangle < Shape
  def initialize(@width : Float64, @height : Float64); end
  def accept(visitor : ShapeVisitor) : Float64; visitor.visit_rectangle(@width, @height); end
end
class AreaVisitor < ShapeVisitor
  def visit_circle(radius : Float64) : Float64; Math::PI * radius * radius; end
  def visit_rectangle(width : Float64, height : Float64) : Float64; width * height; end
end
def visitor_pattern
...
class CounterController
  ...
  def increment; @model.count += 1; end
end
def render_counter(model : CounterModel) : String; "count=#{model.count}"; end
def mvc_pattern
...
class AmountViewModel
  ...
  def text : String; "$#{@amount}.00"; end
  def add(value : Int32); @amount += value; end
end
def mvvm_pattern
...
# Microkernel
class Kernel
  def initialize; @plugins = {} of String => Proc(Int32, Int32); end
  def register(name : String, plugin : Proc(Int32, Int32)); @plugins[name] = plugin; end
  def run(name : String, value : Int32) : Int32; @plugins[name].call(value); end
end
...
class OrderService
  ...
  def place(qty : Int32) : String; @inventory.reserve(qty) ? "confirmed" : "rejected"; end
end
...
def adapt_customer(c : LegacyCustomer) : CanonicalCustomer; CanonicalCustomer.new(c.code, c.cents / 100.0); end
...
class NamedTransport < Transport
  ...
  def send(message : String) : String; "#{@name}>#{message}"; end
end
def send_notice(kind : String, message : String, transport : Transport) : String; transport.send("#{kind}:#{message}"); end
...
class MessageBus
  def initialize; @handlers = [] of Proc(BusMessage, String); end
  def on(handler : Proc(BusMessage, String)); @handlers << handler; end
  def send(message : BusMessage) : Array(String); @handlers.map { |h| h.call(message) }; end
end
...
class MonitoredCounter
  ...
  def initialize; @mutex = Mutex.new; @critical = 0; end
  def add(n : Int32)
...
def server_handle(key : String) : Response; key == "sku-1" ? Response.new(200, "stock=7") : Response.new(404, "missing"); end
...
class Peer
  ...
  def initialize(@name : String); end
  def send(other : Peer, data : String); other.inbox << "#{@name}>#{other.name}:#{data}"; end
end
...
# Publish-Subscribe
class PubSub
  def initialize; @topics = Hash(String, Array(Proc(Int32, String))).new { |h, k| h[k] = [] of Proc(Int32, String) }; end
  def subscribe(topic : String, subscriber : Proc(Int32, String)); @topics[topic] << subscriber; end
  def publish(topic : String, id : Int32) : Array(String); @topics[topic].map { |s| s.call(id) }; end
end
...
class RemoteStock < StockService
  def stock(sku : String) : Int32; 7; end
end
class StockProxy < StockService
  def initialize(@remote : StockService); end
  def stock(sku : String) : Int32; @remote.stock(sku); end
end
...
struct PacAgent
  ...
  def view : String; "#{name}:view=#{value}"; end
end
...
class Presenter
  ...
  def increment; @model.count += 1; @view.text = render_counter(@model); end
end
...
def editor_view(d : Document) : String; "editor:#{d.title}:#{d.words}"; end
def summary_view(d : Document) : String; "summary:#{d.title}"; end
...
class PersonRecord
  ...
  def save; @@table[@id] = self; end
  def self.load(id : Int32) : PersonRecord?; @@table[id]?; end
end
...
class PersonMapper
  def to_row(p : Person) : PersonRow; PersonRow.new("person:#{p.id}", p.name); end
  def from_row(r : PersonRow) : Person; Person.new(8, r.name); end
end
...
class UnitOfWork
  ...
  def initialize(@values : Array(Int32)); @changes = [] of Tuple(Int32, Int32); end
  def stage(index : Int32, delta : Int32); @changes << {index, delta}; end
  def commit
...
class PersonRepository
  ...
  def by_id(id : Int32) : Person?; @items[id]?; end
end
...
class Greeter
  ...
  def greet(name : String) : String; @sender.call(name); end
end
...
# Object Pool
class ObjectPool
  def initialize; @available = [] of Int32; @next_id = 0; end
  def acquire : Int32
    return @available.pop unless @available.empty?
    @next_id += 1
  end
  def release(value : Int32); @available << value; end
end
...
class RealLogger < Logger
  def log(message : String) : String; "logged:#{message}"; end
end
class NullLogger < Logger
  def log(message : String) : String; ""; end
end
